/* eslint-disable react/no-unknown-property */
import React, { useState } from "react";
import Image from "next/image";
import Link from "next/link";

const NoticePage = () => {
  // Sample notices array
  const [notices] = useState([
    {
      id: 1,
      title: "School Reopening Notice",
      content:
        "We are pleased to announce that the school will reopen on March 1, 2025. Please check your emails for further instructions.",
      date: "2025-02-28",
    },
    {
      id: 2,
      title: "Examination Schedule Released",
      content:
        "The examination schedule for the upcoming term has been released. Please refer to the notice for details and timings.",
      date: "2025-02-25",
    },
    {
      id: 3,
      title: "Holiday Announcement",
      content:
        "A holiday has been declared on April 1, 2025, due to regional festivities. Kindly plan your academic activities accordingly.",
      date: "2025-02-20",
    },
  ]);

  const [openAccordion, setOpenAccordion] = useState(null);

  const toggleAccordion = (id) => {
    setOpenAccordion(openAccordion === id ? null : id);
  };

  // Quick options array
  const quickOptions = [
    { name: "Old Notices", link: "/old-notices" },
    { name: "Exam Schedule", link: "/exam-schedule" },
    { name: "CBSE Guidelines", link: "/cbse-guidelines" },
    { name: "Academic Calendar", link: "/academic-calendar" },
  ];

  return (
    <div className="container">
      {/* Left Column - Notices Accordion */}
      <div className="leftColumn">
        <h2 className="leftHeading">Notices</h2>
        <div className="accordion">
          {notices.map((notice) => (
            <div key={notice.id} className="accordionItem">
              <div
                className="accordionHeader"
                onClick={() => toggleAccordion(notice.id)}
              >
                <span className="accordionTitle">{notice.title}</span>
                <i
                  className={`fa-solid fa-chevron-down accordionIcon ${
                    openAccordion === notice.id ? "rotate" : ""
                  }`}
                ></i>
              </div>
              <div
                className="accordionBody"
                style={{
                  display: openAccordion === notice.id ? "block" : "none",
                }}
              >
                <p>{notice.content}</p>
                <small>Date: {notice.date}</small>
              </div>
            </div>
          ))}
        </div>
      </div>

      {/* Right Column - Quick Options */}
      <div className="rightColumn">
        <h2 className="rightHeading">Quick Links</h2>
        <div className="optionsContainer">
          {quickOptions.map((option, index) => (
            <Link key={index} href={option.link} passHref legacyBehavior>
              <a className="optionLink">{option.name}</a>
            </Link>
          ))}
          <button className="moreButton">More</button>
        </div>
      </div>

      <style jsx>{`
        .container {
          max-width: 1200px;
          margin: 0 auto;
          padding: 40px 20px;
          font-family: Arial, sans-serif;
          display: flex;
          gap: 20px;
        }
        .leftColumn {
          flex: 0 0 70%;
        }
        .rightColumn {
          flex: 0 0 30%;
        }
        .leftHeading {
          font-size: 28px;
          margin-bottom: 20px;
          color: #333;
          font-weight: bold;
        }
        .accordion {
          margin-top: 20px;
        }
        .accordionItem {
          margin-bottom: 10px;
          border: 1px solid #ddd;
          border-radius: 5px;
          overflow: hidden;
        }
        .accordionHeader {
          background-color: #f8f9fa;
          padding: 15px;
          cursor: pointer;
          display: flex;
          justify-content: space-between;
          align-items: center;
        }
        .accordionTitle {
          font-size: 18px;
          font-weight: bold;
          color: #333;
        }
        .accordionIcon {
          font-size: 1.2rem;
          transition: transform 0.3s ease;
        }
        .accordionIcon.rotate {
          transform: rotate(180deg);
        }
        .accordionBody {
          padding: 15px;
          font-size: 16px;
          background-color: #fff;
        }
        .rightHeading {
          font-size: 24px;
          margin-bottom: 20px;
          color: #333;
          font-weight: bold;
        }
        .optionsContainer {
          display: flex;
          flex-direction: column;
          gap: 10px;
          margin-top: 20px;
        }
        .optionLink {
          text-decoration: none;
          color: #3d6ce7;
          font-size: 16px;
          padding: 10px;
          border: 1px solid #ddd;
          border-radius: 5px;
          background-color: #f8f9fa;
          text-align: center;
        }
        .moreButton {
          padding: 10px;
          border: none;
          border-radius: 5px;
          background-color: #3d6ce7;
          color: #fff;
          cursor: pointer;
          font-size: 16px;
          margin-top: 10px;
        }
        /* Responsive Styles */
        @media (max-width: 768px) {
          .container {
            flex-direction: column;
          }
          .leftColumn,
          .rightColumn {
            flex: 0 0 100%;
          }
        }
      `}</style>
    </div>
  );
};

export default NoticePage;
